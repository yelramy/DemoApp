...
export default async function AppHome() {
  ...
  return (
    <div className="container-bridge grid gap-8 py-10 lg:grid-cols-[1.1fr_0.9fr]">
      <section className="panel p-6 md:p-8">
        <h1 className="display mb-2 text-3xl text-[var(--sea-deep)]">
          Hi {user.name?.split(" ")[0] ?? "there"}
        </h1>
        <p className="mb-6 text-sm text-[var(--ink)]/70">
          Suite ID for ship-for-me: <strong>{user.suiteCode}</strong>
        </p>
        <QuoteForm />
      </section>
      <section className="panel p-6 md:p-8">
        ...
        <div className="space-y-3">
          {orders.length === 0 ? (
            <p className="text-sm text-[var(--ink)]/60">No orders yet. Paste a link to start.</p>
          ) : (
            orders.map((o) => (
              <Link
                key={o.id}
